...
def main():
    # 입력 폴더와 출력 폴더 설정
    input_folder = r"D:\Straight_Data_0623\Warn"
    output_folder = r"D:\KHB_Videos\0623\Warn"
    ...
    for input_file in input_files:
        ...
        while True:
            ...
            if ret:
                ...
                cv.putText(frame, ('A T: ' + str(TA) + ' vs ' + str(out_ETA_A) + ' s'), (400, 30),
                           cv.FONT_HERSHEY_SIMPLEX, 0.5, WHITE, 2)
                cv.putText(frame, ('A S: ' + str(int(distCA)) + ' S'), (400, 50), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                           WHITE, 2)
                cv.putText(frame, ('A LV: ' + str(Ams) + 'm/s ' + str(Akh) + 'k/h'), (400, 70), cv.FONT_HERSHEY_SIMPLEX,
                           0.5,
                           WHITE, 2)
                ...
                cv.putText(frame, ('B T: ' + str(TB) + ' vs ' + str(out_ETA_B) + ' s'), (800, 30),
                           cv.FONT_HERSHEY_SIMPLEX, 0.5, WHITE, 2)
                cv.putText(frame, ('B S: ' + str(int(distCB)) + ' S'), (800, 50), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                           WHITE, 2)
                cv.putText(frame, ('B LV: ' + str(Bms) + ' m/s ' + str(Bkh) + 'k/h'), (800, 70),
                           cv.FONT_HERSHEY_SIMPLEX, 0.5,
                           WHITE, 2)
                ...
                cv.putText(frame, ('Situation: ' + str(situation)), (550, 240), cv.FONT_HERSHEY_SIMPLEX, 0.8,
                           BLUE, 2)
                    ...
                    if CLS == 2:
                        ...
                        if A < 0:
                            if str(track_id) not in l_i_o_A and str(track_id) in list_id_inA:
                                ...
                                with open(csv_file, mode='a', newline='') as file:
                                    writer = csv.writer(file)
                                    writer.writerow([colum_passA, A_slope, B_slope, distCA, distCB, Ax, Bx, '1'])
                        ...
                        if B < 0:
                            if str(track_id) not in l_i_o_B and str(track_id) in list_id_inB:
                                ...
                                with open(csv_file, mode='a', newline='') as file:
                                    writer = csv.writer(file)
                                    writer.writerow([colum_passB, A_slope, B_slope, distCA, distCB, Ax, Bx, '1'])
