...
def main():
    # 입력 폴더와 출력 폴더 설정
    input_folder = r"D:\Straight_Data_0623\Danger"
    output_folder = r"D:\KHB_Videos\0624\Danger"
                    ...
                    if CLS == 2:
                        ...
                        if A < 0:
                            if str(track_id) not in l_i_o_A and str(track_id) in list_id_inA:
                                ...
                                with open(csv_file, mode='a', newline='') as file:
                                    writer = csv.writer(file)
                                    writer.writerow([colum_passA, A_slope, B_slope, distCA, distCB, Ax, Bx, '2'])
                        ...
                        if B < 0:
                            if str(track_id) not in l_i_o_B and str(track_id) in list_id_inB:
                                ...
                                with open(csv_file, mode='a', newline='') as file:
                                    writer = csv.writer(file)
                                    writer.writerow([colum_passB, A_slope, B_slope, distCA, distCB, Ax, Bx, '2'])
